...
def parse_args():
    ...
    parser.add_argument(
        "--exp_name",
        type=str,
        help="Experiment name to be given via terminal",
        default="baseline",
    )    
    parser.add_argument(
        "--model_name_or_path",
...
def ensemble_prediction(test_example, 
    ...
    if aggregation_method == "confidence":
        ...
        label = test_example["labels"][0].item()
        
        # task_preds = torch.zeros(len(task_models),test_example['labels'].shape[0])
        task_preds = torch.zeros(len(task_models))
        task_conf = torch.zeros(len(task_models))
        if first_test_example:
            print('True label:', label)
            print('task_preds shape like this:', task_preds.shape)

        for itr, task in enumerate(task_models):
            model = task_models[task]
            with torch.no_grad():
                outputs = model(**test_example)
                # print("Model outputs", outputs)
            predictions = outputs.logits.argmax(dim=-1)
            confidence,_ = outputs.logits.max(dim=-1)
            # if first_test_example:            
            #     print("TASK NUMBER:", itr)
            #     print(('confidence:', confidence),('predictions', predictions))            
            task_preds[itr] = predictions
            task_conf[itr] = confidence
        
        conf_indices = torch.argmax(task_conf, dim=0)
        final_preds = task_preds[conf_indices]
        if first_test_example:            
            print("**************************")
            print('Task with highest confidence', conf_indices)  
            print('all task_conf', task_conf)  
            print('all task_preds', task_preds)
            print('selected final_preds', final_preds)
        ## Reshape preds as BATCH_SIZE X 1        
        final_preds = final_preds.reshape([test_example['labels'].shape[0], 1])
        ...
        label = test_example["labels"][0].item()        
        task_preds = torch.zeros(len(task_models))
        task_entropy = torch.zeros(len(task_models))
        ...
        for itr, task in enumerate(task_models):
            ...
            with torch.no_grad():
                outputs = model(**test_example)
            predictions = outputs.logits.argmax(dim=-1)
            pred_probs = F.softmax(outputs.logits, dim=-1)
            
            # if first_test_example:
            #     print(('pred_probs:', pred_probs), pred_probs.shape)
            
            task_preds[itr] = predictions
            task_entropy[itr] = torch.distributions.Categorical(probs=pred_probs).entropy()

        entropy_indices = torch.argmin(task_entropy, dim=0)        
        final_preds = task_preds[entropy_indices]
        if first_test_example:            
            print('Task with lowest entropy', entropy_indices)  
            print("first_test_sample")
            print('all task_entropy', task_entropy)  
            print('all task_preds', task_preds)
            print(("True GT", label),('selected final_preds', final_preds))
...
def main():
    ...
    if args.with_tracking:
        print("We will be tracking logs in wandb.")
    ...
    # task_split_meta = read_json("./task_splits_random_overlap.json")
    # task_split_meta = read_json("./task_splits_superclass.json")
    ...
    ## T0: Evaluate all individual models on combined eval dataset
    ...
    ## 2. Oracle scheme Evaluation: check which task the val image's label belongs to, then use that task's model to predict
    ...
    for aggregation_method in ["confidence", "entropy"]:#, "neighbor_majority"]:
        for step, batch in tqdm(enumerate(combined_eval_dataloader_bsz_1), desc=f"Evaluating ensemble on combined eval dataset (aggregation='{aggregation_method}')"):            
            if step==0: first_test_example = True

            prediction = ensemble_prediction(batch, task_models, aggregation_method, label_to_task=label_to_task, first_test_example=first_test_example)
            ...
            # if step==0:
            #     print(("batch contains at step", step), batch.keys())
            #     break;
