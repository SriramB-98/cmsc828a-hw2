...
class FedAvgServer:
    def __init__(
        ...
        with open(_PROJECT_DIR / "data" / self.args.dataset / self.args.partition / "args.json", "r") as f:
        ...
        try:
            partition_path = _PROJECT_DIR / "data" / self.args.dataset / self.args.partition / "partition.pkl"
    ...
    def run(self):
        ...
        # save log files
        if not os.path.isdir(OUT_DIR / self.algo / self.prefix) and (
            self.args.save_log or self.args.save_fig or self.args.save_metrics
        ):
            os.makedirs(OUT_DIR / self.algo / self.prefix , exist_ok=True)

        if self.args.save_log:
            self.logger.save_text(OUT_DIR / self.algo /  self.prefix / f"{self.args.dataset}_{self.args.partition}_jr-{self.args.join_ratio}_log.html")
        ...
        if self.args.save_fig:
            ...
            plt.title(f"{self.algo}_{self.args.dataset}_{self.args.partition}_jr-{self.args.join_ratio}")
            plt.ylim(0, 100)
            plt.xlabel("Communication Rounds")
            plt.ylabel("Accuracy")
            plt.legend()
            plt.savefig(
                OUT_DIR / self.algo /  self.prefix / f"{self.args.dataset}_{self.args.partition}_jr-{self.args.join_ratio}.jpeg", bbox_inches="tight"
            )
        ...
        if self.args.save_model:
            model_name = (
                f"{self.args.dataset}_{self.args.partition}_jr-{self.args.join_ratio}_{self.args.global_epoch}_{self.args.model}.pt"
            )
            if self.unique_model:
                torch.save(
                    self.client_trainable_params, OUT_DIR / self.algo /  self.prefix / model_name
                )
            else:
                torch.save(self.model.state_dict(), OUT_DIR / self.algo /  self.prefix /  model_name)
